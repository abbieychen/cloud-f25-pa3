...
class BatchPublisher:
    def __init__(self, broker_list, batch_size=10, topic='sensor-data'):
        ...
        logger.info(f"Publisher 3 initialized with {len(broker_list)} brokers")
    ...
    def publish_data(self, data_file):
        ...
        try:
            ...
            logger.info(f"Publisher 3: Loaded {total_records} records from {data_file}")
            ...
            for _, row in df.iterrows():
                ...
                success_count += 1
                logger.info(f"Publisher 3: Successfully sent record {record['id']} ({success_count}/{total_records})")
                time.sleep(0.2)

            # Wait for all messages to be delivered
            self.producer.flush()
            logger.info(f"Publisher 3: Completed publishing {success_count} records")

        except Exception as e:
            logger.error(f"Publisher 3: Error publishing data: {e}")
            raise
...
def main():
    ...
    logger.info(f"Publisher 3: Starting with brokers: {KAFKA_BROKERS}")
    logger.info(f"Publisher 3: Data file: {DATA_FILE}")

    # Wait for Kafka to be ready
    logger.info("Publisher 3: Waiting for Kafka brokers to be ready...")
    if not wait_for_kafka(KAFKA_BROKERS, max_retries=40, retry_interval=5):
        logger.error("Publisher 3: Failed to connect to Kafka brokers after multiple attempts")
        return
